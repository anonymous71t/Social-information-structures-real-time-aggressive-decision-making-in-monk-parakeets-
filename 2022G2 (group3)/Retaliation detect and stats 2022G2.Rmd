---
title: "Retaliation detect and stats 2022G2"
author:
date: 
...
practdrlist_ordered2022G2 <- list()
...
for(i in datex) { 
  ...
  output <- mutate(output, counter = actorer[g])
      practdrlist_ordered2022G2[[paste0("list", dater[i], actorer[g])]] <- output}}
...
practdrlist_ordered2022G2 <- lapply(practdrlist_ordered2022G2, function(x) arrange(x, date, full_time))
...
#Global threshold calculation pt3 of 3
```{r}
...
time_between_events3 <- subset(testhugesorted_rule_time, select = c(time_to_retal))
time_between_events3$time_to_retal <- as.numeric(time_between_events3$time_to_retal)



mean_time_between_events3 <- mean(time_between_events3$time_to_retal, na.rm=TRUE)


ggplot(time_between_events3, aes(x=time_to_retal)) + 
  geom_histogram(aes(y=..density..),      
                 binwidth=10,
                 colour="black", fill="white") + 
...
#write.csv(masterframe_retaliation.df_time_2022G2,"retal2022G2timemaster.csv", row.names = FALSE)
...
```{r}
#masterframe_retaliation.df_time_2022G2 <- read_csv("retal2022G2timemaster.csv")
...
median_obs_rand_retaliation_2022G2 <- retal_length_ready_2022G2_comb %>% group_by(data_type) %>%
  dplyr::summarize (median(time_to_retal))

median_obs_rand_retaliation_2022G2
...
m1retal <- glmer(time_to_retal ~ data_type  + (1 | date_period) , data = retal_length_ready_2022G2_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
summary(m1retal)
...
#ensure that randomized speed is numeric
...
#determine how many times response motif was executed
...
pb = txtProgressBar(min=0, max = nruns, style = 3)
#forloop starts here'
#seed for 1273 sec = 947
#seed for  194 sec = 9
#seed for 1363 sec = 66 
#seed for 414 sec = 87
set.seed(947)
...
#sort by time and date
practdrlist_ordered2022G2 <- lapply(practdrlist_split_by_bird, function(x) arrange(x, date, full_time))
...
#write.csv(masterframe_retaliation.df2022G2,  "retal2022G2countmaster_rule_spec_95perc.csv", row.names = FALSE)
...
#masterframe_retaliation.df2022G2 <- read_csv("retal2022G2countmaster_rule_spec_95perc.csv")
...
retal_2022G2_count_plot <- ggplot(rand_retal_.frame2022G2, aes(x=random_times_retaliation_followed)) +
  ...
  geom_vline(xintercept = retal_real2022G2, colour = "red", size = 2)  + theme(axis.text = element_text(size=25), axis.title = element_text(size = 25, face = "bold")) +
...
retal_tally2022G2 <-length(which(retal_real2022G2 > rand_retal_.frame2022G2$random_times_retaliation_followed ))
...
length(unique(t2022_aggression_only_readyG2$actor))
length(unique(t2022_aggression_only_readyG2$subject))


retal_nofollow <- setdiff(unique(c(t2022_aggression_only_readyG2$actor, t2022_aggression_only_readyG2$subject)), unique(retal_doner_all_yes$actor_2)
 )
