---
title: "Passalong detect and stats 2022G2"
author: 
date: 
...
#put df side by side
pass_along_side_by_side_time <- lapply(pass_along_aggression_list_2022G2, function(x) mutate(x, actor_2 = lead(actor), actor_2_rank = lead(actor_rank),   subject_2 = lead(subject), subject_2_rank = lead(subject_rank), full_time_2 = lead(full_time), date2 = lead(date)))
...
#write.csv(masterframe_passalong.df_time_2022G2, "passalong2022G2timemaster.csv", row.names = FALSE)
...
#masterframe_passalong.df_time_2022G2 <- read_csv("passalong2022G2timemaster.csv")
...
m1pass_2022G2 <- lme4::glmer(time_to_pass ~ data_type  + (1 | date_period) , data = pass_length_ready_2022G2_comb ,   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)), family = Gamma(link = "log"))
...
pass_along_side_by_side <- lapply(pass_along_aggression_list_2022G2, function(x) mutate(x, actor_2 = lead(actor), actor_2_rank = lead(actor_rank), subject_2 = lead(subject),subject_2_rank = lead(subject_rank), full_time_2 = lead(full_time), date2 = lead(date))) 
...
#set seed
set.seed(270)
#seed for 1476 sec = 270
#seed for 494 sec = 287
#seed for 414 = 57
# seed for 1363 = 56 
#forloop starts here'
for (r in 1:length(runIDs)) { #r = 1 is a test you can uncomment to see if it works before #running it 1000 times
...
#write.csv(masterframe_passalong_2022G2.df, "passalong2022G2countmaster_rule_spec_95perc.csv", row.names = FALSE)
...
#masterframe_passalong_2022G2.df <- read_csv("passalong2022G2countmaster_rule_spec_95perc.csv")
...
pass_along_nofollow <- setdiff(unique(t2022_aggression_only_readyG2$actor), unique(pass_along_doner_all_yes$actor_2)
)
pass_along_nofollow
