---
title: "Passalong detect and stats 2021"
author: 
date: 
...
#put df side by side
pass_along_side_by_side_time <- lapply(pass_along_aggression_list, function(x) mutate(x, actor_2 = lead(actor), actor_2_rank = lead(actor_rank), subject_2 = lead(subject),  subject_2_rank = lead(subject_rank), full_time_2 = lead(full_time), date2 = lead(date))) 
...
pass_along_side_by_side_time <- lapply(practdrlist_ordered, function(x) mutate(x, actor_2 = lead(actor), actor_2_rank = lead(actor_rank), subject_2 = lead(subject),  subject_2_rank = lead(subject_rank), full_time_2 = lead(full_time),  date2 = lead(date))) 
...
#write.csv(masterframe_passalong.df_time_2021, "passalong2021timemaster.csv", row.names = FALSE)
...
```{r}
#masterframe_passalong.df_time_2021 <- read_csv("passalong2021timemaster.csv")
...
#calculate median of motif speeds observed vs randomized
...
#ensure that randomized motif execution count is numeric
...
#write.csv(masterframe_passalong.df, "passalong2021countmaster_rule_spec_95perc.csv", row.names = FALSE)
...
#masterframe_passalong.df <- read_csv("passalong2021countmaster_rule_spec_95perc.csv")
